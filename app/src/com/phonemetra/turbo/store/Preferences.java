...
/**
 * Handles shared preferences for TurboStore, looking after the names of
 * preferences, default values and caching. Needs to be setup in the StoreApp
 * (using {@link Preferences#setup(android.content.Context)} before it gets
 * accessed via the {@link com.phonemetra.turbo.store.Preferences#get()}
 * singleton method.
 */
    ...
    public static void setup(Context context) {
        if (instance != null) {
            final String error = "Attempted to reinitialize preferences after it " +
                    "has already been initialized in StoreApp";
            Log.e(TAG, error);
    ...
    public static Preferences get() {
        if (instance == null) {
            final String error = "Attempted to access preferences before it " +
                    "has been initialized in StoreApp";
            Log.e(TAG, error);
            throw new RuntimeException(error);
        }
        return instance;
